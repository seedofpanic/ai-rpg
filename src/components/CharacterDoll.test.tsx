...
describe('CharacterDoll', () => {
  ...
  mockMagicEffects.applyEffect('player', {
    type: 'heal',
    value: 10,
    source: 'Potion',
  });
  ...
  it('renders all equipment slots', () => {
    ...
    expect(screen.getByTestId('equipment-slot-weapon')).toHaveTextContent(
      'Steel Sword',
    );
    expect(screen.getByTestId('equipment-slot-shield')).toHaveTextContent(
      'Wooden Shield',
    );
    expect(screen.getByTestId('equipment-slot-chest')).toHaveTextContent(
      'Leather Armor',
    );
  });
  ...
  it('shows empty state when no active effects', () => {
    const playerWithoutEffects = new Player(
      'Test Player',
      'Human',
      'Warrior',
      'player',
    ...
    expect(screen.getByTestId('no-effects-message')).toHaveTextContent(
      'No active effects',
    );
  });
