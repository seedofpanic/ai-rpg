...
test.describe('AI RPG E2E Tests', () => {
  test('should load the game and allow character customization', async ({ page }) => {
    // Verify the game map is loaded
    await page.locator('[data-testid="player-view"]').waitFor({ state: 'visible' });
  });
  ...
  test('should ask for trade and see the items in trade', async ({ page }) => {
    await page.route('https://generativelanguage.googleapis.com/**', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify(mockGeminiResponse('Mocked AI response <sell>Sword, 1; helm, 2;</sell>')),
      });
    });
    ...
    const item = page.locator('[data-testid="trade-item"]', { hasText: "Sword"});
    await item.waitFor({ state: 'visible' });

    // Buy sword
    await item.locator('[data-testid="buy-item"]').click();
    await item.waitFor({ state: 'hidden' });
    await page.locator('[data-testid="player-inventory"]').locator('[data-testid="item-veiw"]', { hasText: "Sword"}).waitFor({ state: 'visible' });
